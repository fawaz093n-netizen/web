...
When prompted, choose PostgreSQL. Pick Prisma Postgres if you want setup to create the database for you, or paste your own `DATABASE_URL`.
...
- Node.js 24 or newer
- npm
- a PostgreSQL database, or Prisma Postgres from the setup prompt
...
`db:init` applies the starter schema to PostgreSQL. `db:seed` inserts sample users so the first query has data to show.

## 3. Run the app

Use the URL or terminal output shown by your template to confirm the sample query runs successfully. You should see the seeded users returned from PostgreSQL.
