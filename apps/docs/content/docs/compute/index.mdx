...
Enable SSR with the Node adapter in standalone mode in `astro.config.mjs`:
...
Scaffold from the official TanStack Start starter (Vite + Nitro `node` preset) and `cd` into the project.
