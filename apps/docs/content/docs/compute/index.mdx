...
The first deploy bootstraps project context. Re-deploys reuse the saved selection — `app deploy` (or `deploy`) is enough.
...
```bash
npx @looma/prisma-cli app update-env --env STRIPE_KEY=sk_live_…
...
```bash
bunx @prisma/compute-cli env update --env STRIPE_KEY=sk_live_…
...
- `env update` / `app update-env` cuts a new compute version — old version stays live until the new one is healthy.
- No bulk `.env` import yet — repeat `--env KEY=VALUE`.
