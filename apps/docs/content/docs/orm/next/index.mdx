...
:::warning
Prisma Next is the path toward the next major version of Prisma ORM (Prisma 8). Its APIs are still evolving and it is not ready for production, so [Prisma ORM 7](/orm) remains the right choice for production applications today. Share feedback in [Discord](https://pris.ly/discord) as the design takes shape.
:::
