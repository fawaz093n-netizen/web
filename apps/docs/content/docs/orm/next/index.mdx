...
Use [Getting Started](/next) when you want setup instructions. The setup flow uses `create-prisma@next` to scaffold a complete application template and add package scripts for contract emission, database initialization, verification, and migrations.

Use the [Prisma Next CLI reference](/cli/next) when you need exact command flags.

For the broader story behind the rewrite, query APIs, extensibility, migrations, and agent-friendly workflows, read [The Next Evolution of Prisma ORM](https://www.prisma.io/blog/the-next-evolution-of-prisma-orm).
