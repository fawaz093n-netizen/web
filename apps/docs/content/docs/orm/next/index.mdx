...
Prisma Next is the next major version of Prisma ORM, available now in Early Access. Its APIs are still evolving and will change, so [share your feedback in Discord](https://pris.ly/discord) to help shape the design. Prisma 7 is the current generally available release, so if you are building for production, keep using [Prisma ORM 7](/orm).
:::
