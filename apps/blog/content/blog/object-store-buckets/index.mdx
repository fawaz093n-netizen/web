---
title: "Your Agent Can Provision File Storage Now: Object Store Buckets in Every Prisma Project"
...
Every Prisma project can now store files. Object Store buckets are S3-compatible storage that lives alongside your [Prisma Postgres](https://www.prisma.io/docs/postgres) databases, managed from the Prisma Console, the [Management API](https://www.prisma.io/docs/management-api), or, per the changelog, matching MCP tools. Two API calls take you from nothing to reading and writing objects, which means a coding agent, handed a service token once, can provision storage the same way it already provisions databases: mid-task, without waiting for a human to click through a dashboard.

This post covers what shipped, then walks the exact path an agent takes: create a bucket, mint a key, write a file, serve it, and tear everything down, every step an API call, with the real output from our runs.
...
That stop is what the [July 24 changelog](https://www.prisma.io/changelog/2026-07-24) removes: storage is now a resource inside a Prisma project, next to the database, under the same account, on the same API surface.

## What shipped

Object Store buckets, available in every Prisma project per the [changelog](https://www.prisma.io/changelog/2026-07-24):

- **S3-compatible.** Buckets speak the S3 API; the changelog puts it as working with any S3 client or SDK, and our runs verified Bun's built-in S3 client end-to-end. There is no proprietary client to learn.
- **Managed from the Console and the API.** The Prisma Console for humans, and the `/v1/buckets` [Management API endpoints](https://www.prisma.io/docs/management-api/endpoints/buckets/get-buckets) for scripts and agents. The changelog announces matching MCP tools alongside; the MCP server docs' tool list doesn't include them yet at the time of writing.
- **Access keys with roles.** Keys are minted per bucket with a `read` or `read_write` role. The `secretAccessKey` is returned exactly once and is not retrievable afterward, as the API reference's own warning puts it; capturing it into your environment at mint time is your half of the handshake.
- **Branch-aware.** A bucket belongs to a project and can be associated with a branch at creation, via `branchId` or `branchGitName` (the list endpoint also accepts `branchId=unassigned`, so buckets without a branch exist in the model). In our runs, omitting both fields attached the bucket to the project's default branch.
...
Object Store pricing and plan limits are not published in the docs at the time of writing; watch the [changelog](https://www.prisma.io/changelog) for updates before an agent provisions storage unattended.
...
Authentication is a [service token](https://www.prisma.io/docs/management-api/authentication), created once in the Console (your workspace, then **Settings → Service Tokens**) and handed to the agent as an environment variable. Set `PRISMA_SERVICE_TOKEN` and `PRISMA_PROJECT_ID` in a `.env` file; the project id sits in the project's Console URL, or one `GET /v1/projects` call away. (An agent starting from an empty workspace can create the project too, with `POST /v1/projects`; we scope this walkthrough to an existing project.)

Everything below was run in July 2026 with Bun 1.3 against the live Management API.
...
The bucket came back with `status: ready` and, in our runs, a `branchId` nobody asked for. Listing the project's branches (`GET /v1/projects/{projectId}/branches`) confirmed that id is the project's default branch (`isDefault: true`). There was no propagation delay to wait out; the first S3 write, moments later, succeeded.

The key-minting response is the entire configuration handoff for the client we ran: `accessKeyId`, `secretAccessKey`, `endpoint`, `bucketName` map one-for-one onto the constructor of Bun's built-in `S3Client`, with a single rename (`bucketName` becomes the `bucket` option). Other S3 clients accept the same values in their own configuration shapes; check your client's S3-compatible-endpoint setup for the exact spelling. An agent does not need to assemble configuration from three dashboard pages.

The served file is real. The presigned URL answered an unauthenticated `GET` with HTTP 200 and the exact body: a time-boxed link (an hour here) your app can hand to a browser without shipping credentials to the client. We exercised the signed path only; verify your bucket's unsigned access posture yourself before relying on it for private files.
...
And the teardown deleted a bucket that still had an object in it. That is the changelog's one-step deletion exercised, not just quoted, and per the delete endpoint's reference it takes the bucket's access keys with it too: no empty-the-bucket-first ritual, which cuts both ways, as the next section says.

## Wiring it into your agent

The service token is the one secret a human creates up front; everything downstream is the agent's to manage. Four habits keep that safe and repeatable:

- **Know what the token is before handing it over.** A service token never expires (the authentication docs' own warning), it is minted at workspace level, and it carries the Management API surface that this post just demonstrated, including the one-call, contents-included bucket deletion. That convenience is also the blast radius. Hand the token to the agent as environment, never as chat (`.env` locally, a secret store in CI), give agent work its own workspace so a wrong call lands in a sandbox rather than next to production data, and use one token per purpose so you can retire a token later without breaking everything else.
- **Write minted secrets to the environment, not the transcript.** The `secretAccessKey` a key mint returns is shown exactly once and is not retrievable afterward. Route it straight into the environment of whatever will use it: any surface that echoes the mint response into a conversation transcript, an MCP tool result included, has already leaked it. The changelog announces matching MCP bucket tools, and the [MCP server docs](https://www.prisma.io/docs/ai/tools/mcp-server)' tool list does not include them yet, which is one more reason this post routes key minting through the API path.
- **Scope the keys you ship, not the agent.** A `read` key is what a file-serving deployment should hold: in our runs it could read objects, and a write with it came back `AccessDenied`. But roles constrain the key, not the agent, because an agent holding the service token can mint itself a `read_write` key at any time. Key roles protect the credentials you hand onward, and the service token's containment (the first habit) protects everything else.
...
Branch association is where that points. Prisma Postgres gives branches their own databases, and a bucket can be tied to a branch at creation with `branchId` or `branchGitName`, so a branch environment can carry its files next to its data. The association happens when the bucket is created: pass a branch to choose one, and in our runs, omitting it attached the bucket to the project's default branch. And when the environment goes away, its bucket goes with one `DELETE`, contents included, behind the same human confirmation the previous section put on it.

## Frequently asked questions

<Accordions type="single">
  <Accordion title="Are Object Store buckets S3-compatible with existing SDKs and tools?">
Yes. Buckets expose the S3 API, and the changelog states they work with any S3 client or SDK. The key-minting response returns `accessKeyId`, `secretAccessKey`, `endpoint`, and `bucketName`. Bun's built-in client took them nearly verbatim in our runs (its `bucket` option is the response's `bucketName`); other clients shape the same values their own way and may want extras of their own, like a region setting. In our runs, Bun's built-in `S3Client` wrote, read, and served objects (via a presigned URL) against a fresh bucket with no additional configuration.
  </Accordion>
  <Accordion title="How does a coding agent authenticate to provision buckets?">
With a Management API service token, sent as a `Bearer` token in the `Authorization` header. A human creates the token once in the Prisma Console (workspace **Settings → Service Tokens**) and provides it to the agent as an environment variable; from there the agent can create buckets, mint keys, and delete both via the `/v1/buckets` endpoints. The token never expires (per the Management API authentication docs) and reaches well beyond buckets, so give agent work its own workspace. The changelog also announces matching MCP tools for bucket management; the MCP server docs' tool list does not include them yet, which is why this post recommends the API path.
  </Accordion>
  <Accordion title="What roles do bucket access keys support?">
Two, per the Management API schema: `read` and `read_write`, set when the key is minted via `POST /v1/buckets/{bucketId}/keys`. The roles are enforced by the storage layer: in our runs, a `read` key could fetch objects while a write with it was rejected with `AccessDenied`. The response includes the `secretAccessKey` exactly once; it is not retrievable afterward, so capture it into your environment at mint time. Keys can be revoked individually with `DELETE /v1/buckets/{bucketId}/keys/{keyId}`.
  </Accordion>
  <Accordion title="Do buckets work with database branches?">
Yes. A bucket belongs to a project and can be associated with a branch at creation by passing `branchId` or `branchGitName`; the list endpoint also accepts `branchId=unassigned`, so buckets without a branch exist in the model. In our runs, creating a bucket with neither field attached it to the project's default branch. Paired with per-branch Prisma Postgres databases, this lets a branch environment carry its files next to its data.
  </Accordion>
</Accordions>

## Recap

Object Store buckets put S3-compatible file storage inside every Prisma project: created from the Console or the Management API (with MCP tools announced alongside), secured with per-bucket keys in `read` and `read_write` roles enforced at the storage layer, branch-associable, served over presigned URLs, and deleted contents-and-all in one call.

For agents, the shape matters more than the feature. Storage used to be the step where an agent stopped and waited for a human to open an account. Now it is `POST /v1/buckets`, `POST /v1/buckets/{id}/keys`, and four fields into an S3 client, verified end-to-end in the walkthrough above, serving included.
